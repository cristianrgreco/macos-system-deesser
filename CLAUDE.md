...
A macOS Tahoe (26.0+) menu-bar utility that de-esses **only Microsoft Teams** audio in real time. It taps the Teams output process, runs it through a de-esser, and plays it back out the device Teams was using — without touching the mic or any other app. It is a `LSUIElement` accessory (no Dock icon).

The Xcode project lives in the **`TeamsDeEsser/`** subdirectory; run all build commands from there. The repo root holds `spec.md`, `CLAUDE.md`, and the project folder.
...
xcodebuild -project TeamsDeEsser.xcodeproj -scheme TeamsDeEsser -configuration Debug -destination 'platform=macOS' build
xcodebuild -project TeamsDeEsser.xcodeproj -scheme TeamsDeEsser -destination 'platform=macOS' test
xcodebuild -project TeamsDeEsser.xcodeproj -scheme TeamsDeEsser -configuration Release -destination 'platform=macOS' analyze

# Single test / single test case:
xcodebuild ... test -only-testing:TeamsDeEsserTests/DeEsserDSPTests
xcodebuild ... test -only-testing:TeamsDeEsserTests/DeEsserDSPTests/testReleaseRecovers
```
...
2. **`AudioPipelineCoordinator`** (`Audio/`, serial `DispatchQueue` `local.TeamsDeEsser.control`). The brain. **All Core Audio graph mutation and all `ProcessingState` transitions happen on this one queue** — process discovery, output-device resolution, tap + private aggregate-device construction, the property-change monitor, the heartbeat watchdog, and debounced rebuilds. Treat the queue confinement as invariant.
...
Teams output process → Core Audio process tap (stereo mixdown, `mutedWhenTapped`) → **private aggregate device** → `TDRealtimeRenderer` IOProc → `td::DeEsserDSP` → the physical output device Teams was using. The design is **fail-open**: the tap mutes Teams only while it is being read, so if any layer tears down, normal unprocessed Teams audio resumes automatically.
...
One user control. `DeEsserSettings.strength` (0…1) maps **threshold + ratio together** to the Calf controls (piecewise: 0.5 = the stock EasyEffects defaults; the upper half ramps to a deliberately heavy −42 dBFS / 12:1). Every other Calf control is pinned to its EasyEffects default. The flow is: `DeEsserSettings.rendererParams(bypass:)` → `TDDeEsserParams` (a C struct in `TeamsDeEsser-Bridging-Header.h` / `TDRealtimeRenderer.h`) → `td::DeEsserParams`, where dB fields are converted to the linear values Calf expects (mirroring EasyEffects' `BIND_LV2_PORT_DB`). If you add a tunable parameter, it must be threaded through all four representations.

## Conventions and gotchas

- **`spec.md` is the canonical specification**, section-numbered; code comments reference it as "spec §N". **`IMPLEMENTATION_NOTES.md` records every deviation from the spec, the SDK-name differences, and the engineering rationale** — read it before changing graph lifecycle, the aggregate-device keys, or the DSP, and update it when you deviate further.
- **No `AudioHardwareSystem` / `AudioHardwareProcess` Swift overlay exists** in this SDK; process discovery and device resolution use the **raw C Core Audio property API** (`Audio/CoreAudioProperties.swift`, `Discovery/`). See IMPLEMENTATION_NOTES §1.
- The app is **ad-hoc signed** (`CODE_SIGN_IDENTITY = "-"`), Hardened Runtime **off**, App Sandbox **off**, for frictionless local builds. It requests **only** system-audio capture (`NSAudioCaptureUsageDescription`), never the mic. Keep the bundle id **`local.TeamsDeEsser`** stable — the granted permission is keyed to it.
...
- **The full audio path cannot be verified in a headless build** — it needs a live Teams call and physical device changes. Automated tests cover the DSP and the value-type orchestration only; the manual matrix lives in IMPLEMENTATION_NOTES §5 and spec §18.
