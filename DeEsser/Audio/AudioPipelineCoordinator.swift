...
final class AudioPipelineCoordinator {
    ...
    private let queue = DispatchQueue(label: "local.DeEsser.control")
    ...
    private func enable() {
        ...
        refresh(reason: .environmentChanged, force: true)
    }
    ...
    /// Resolves the output device and builds/rebuilds/tears down as needed.
    /// `force` rebuilds even if the environment fingerprint is unchanged (used
    /// for wake, manual, and heartbeat recovery).
    private func refresh(reason: RebuildReason, force: Bool) {
        guard enabled else { return }

        guard let device = resolver.resolveLive() else {
        ...
        let fingerprint = EnvFingerprint(outputUID: device.uid,
        ...
        buildGraph(device: device, fingerprint: fingerprint)
    }

    // MARK: - Build (spec §5.1)

    private func buildGraph(device: ResolvedOutputDevice, fingerprint: EnvFingerprint) {
        generation &+= 1
        let gen = generation
        transition(to: .starting)

        let excludedProcessIDs = ownProcessObjectIDs()
        Log.audio.notice("""
        ...
        sr=\(device.sampleRate) excludedProcs=\(excludedProcessIDs)
        """)

        do {
            // Build everything without starting read activity (fail-open).
            // Use an unscoped global tap (no device scoping): scoping the tap to
            // the same device the aggregate outputs to prevents the aggregate's
            // I/O from starting (aggIsRunning stayed 0). We exclude our own
            // process so the de-essed replay is not re-captured.
            let tap = try ProcessTapHandle(excludingProcessObjectIDs: excludedProcessIDs, outputDeviceUID: nil)
            ...
            // Register graph-specific listeners. A global tap has no target
            // process set to monitor — only the output device matters.
    ...
    private func handleBuildFailure(_ status: OSStatus, _ context: String, unsupported: Bool = false) {
        ...
        if unsupported {
            ...
            message = "Could not start audio processing"
        }
    ...
    /// Our own HAL audio-process object(s), excluded from the global tap so the
    /// processed audio we replay is never re-captured into it. Returns an empty
    /// list (a fully global tap) if the system has no process object for us yet —
    /// logged, because that risks a feedback loop and is worth seeing.
    private func ownProcessObjectIDs() -> [AudioObjectID] {
        if let id = CA.translatePIDToProcessObject(getpid()) { return [id] }
        Log.audio.notice("TDDIAG could not resolve own audio process object; tap will be fully global")
        return []
    }
    ...
    private func teardownGraph() {
        ...
        // while still enabled so we notice output-route changes.
        monitor.stopAll()
        if enabled {
            monitor.startSystemListeners()
            monitor.startSleepWake()
    ...
    private func recoverFromGraphFailure(reason: RebuildReason) {
        teardownGraph()
        transition(to: .rebuilding(reason: reason))
        // Rebuild once after a short debounce if the output device remains available.
    ...
    private func scheduleRetry() {
        ...
        queue.asyncAfter(deadline: .now() + delay) { [weak self] in
            ...
            self.refresh(reason: .environmentChanged, force: true)
        }
    }
