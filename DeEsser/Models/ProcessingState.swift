...
enum RebuildReason: Equatable {
    ...
    var description: String {
        switch self {
        case .environmentChanged: return "Audio environment changed"
...
enum ProcessingState: Equatable {
    ...
    func statusText(outputDeviceName: String?) -> String {
        ...
        case .running(let summary):
            // Use the verb itself to convey bypass, rather than a trailing
            // "(bypassed)" suffix that pushes the line past the popover width.
            let verb = summary.bypassed ? "Bypassed" : "De-essing"
            return "\(verb) → \(summary.outputDeviceName)"
