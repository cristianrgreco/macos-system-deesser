...
final class ProcessTapHandle {
    ...
    /// Creates a private, `mutedWhenTapped` stereo-mixdown tap of the given Teams
    /// process objects. `outputDeviceUID` scopes the tap to a specific device
    /// when provided; pass `nil` for a plain process-inclusion tap (which avoids
    /// a self-reference when the aggregate also outputs to that device).
    init(processObjectIDs: [AudioObjectID], outputDeviceUID: String?) throws {
        let description = CATapDescription(stereoMixdownOfProcesses: processObjectIDs)
        description.name = "Teams De-Esser Tap"
        ...
        deviceUID=\(outputDeviceUID ?? "(process-tap)", privacy: .public) procs=\(processObjectIDs)
        """)
    }
